...
Each skill reads from and writes to layers relevant to its role. The common pattern and the full targets table live in `references/obsidian-mcp-usage.md#skill-memory--context-pattern` so they are maintained in one place.
